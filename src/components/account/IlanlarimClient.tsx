...
export function IlanlarimClient({ allListings }: { allListings: Listing[] }) {
  ...
  const changeStatus = (id: string, next: ListingStatus) => {
    startTransition(async () => {
      await updateListingStatusAction(id, next);
      router.refresh();
    });
  };
      ...
      {allListings.length > 0 && (
        <div className="space-y-3 rounded-lg border border-line bg-surface p-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <Select value={categorySlug} onChange={(e) => setCategorySlug(e.target.value)} className="sm:w-56">
            ...
            return (
              <div key={l.id} className="flex flex-col gap-4 rounded-lg border border-line bg-surface p-4 sm:flex-row sm:items-center">
                ...
                <div className="flex shrink-0 gap-2">
                  ...
                  {l.status === "aktif" ? (
                    <Button size="sm" variant="ghost" disabled={pending} onClick={() => changeStatus(l.id, "pasif")}>
                      Yayından Kaldır
                    </Button>
                  ) : (
                    <Button size="sm" variant="ghost" disabled={pending} onClick={() => changeStatus(l.id, "aktif")}>
                      Yayınla
                    </Button>
                  )}
                </div>
